...
public class SMPPPythonInterface implements RegionalizationMethod{
    ...
    public void execute_regionalization(Map<Integer, Set<Integer>> neighbor, ArrayList<Long> disAttr, ArrayList<Long> sumAttr, Long threshold) throws ParseException, IOException {
        ...
        ArrayList<DataPartition> dataPartitions = SMPPGRAPH.coarsening(nColumns, nRows, areas, neighbors);
        ...
        HashMap<Integer, HashMap<Integer, List>> partitionsNeighbors = SMPPGRAPH.createNeighborsList(neighbors, dataPartitions);
        ...
        for (int i = 0; i < growResult.size(); i++) {
            ...
            if (p > maxP) {
                ...
                HashMap<Integer, Integer> areasWithRegions = SMPPGRAPH.createAreasWithRegions(partition);
        ...
        for (int i = 0; i < partitionsBeforeEnclaves.size(); i++) {
            ...
            Partition currentPartition = partitionsBeforeEnclaves.get(i);
            Partition feasiblePartition = SMPPGRAPH.enclavesAssignment(population, household, neighbors, currentPartition);
            long heterogeneity = SMPPGRAPH.calculatePartitionH(feasiblePartition, population);
        ...
        bestP = SMPPGRAPH.modifiedSA(lengthTabu, convSA, alpha, t, threshold, bestFeasiblePartition, household, population, neighbors);
