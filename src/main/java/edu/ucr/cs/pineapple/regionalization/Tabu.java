...
import edu.ucr.emp.GMaxPTabu;
import edu.ucr.emp.Move;
...
public class Tabu {
    ...
    public static TabuReturn performTabu(int[] initLabels,
                                    Map<Integer, RegionNew> initRegionList,
                                    SpatialGrid r,
                                    long[][] distanceMatrix,
                                    int tabuLength,
                                    int max_no_move,
        ...
        Map<Integer, RegionNew> regionList = initRegionList;
            ...
            if (make_move_flag || potentialAreas.size() == 0){
                ...
                if (!tabuList.contains(potentialMove)){
                    ...
                    if(debug){
                        ...
                        RegionNew errorRegion = regionList.get(potentialMove.donorRegion);
                    ...
                    if(withinRegionDistance < bestWDS){
                        ...
                        Move prohibitMove = new Move( potentialMove.area, potentialMove.recipientRegion, potentialMove.donorRegion);
                        tabuList.add(prohibitMove);
                        ni_move_ct = 0;
                        if(debug){
                            for(Map.Entry e: regionList.entrySet()){
                                RegionNew tmpr = (RegionNew)e.getValue();
                                ...
                                if(!satisfyable){
                                    ...
                                    RegionNew errorRegion = regionList.get(tmpr.getId());
                                    System.out.println("Area sum " + sumAttr.get(potentialMove.area));
                                    System.out.println("Region ID:" + errorRegion.getId());
                                        System.out.println("Min:" +errorRegion.getMin());
                                        System.out.println("Max:" + errorRegion.getMax());
    ...
    public static List<Integer> pickMoveAreaNew(int[] labels, Map<Integer, RegionNew> regionList, SpatialGrid r, long[][] distanceMatrix, ArrayList<Long> minAttr, ArrayList<Long> maxAttr, ArrayList<Long> avgAttr, ArrayList<Long> sumAttr){
        List<Integer> potentialAreas = new ArrayList<Integer>();
    ...
    public static List<Integer> pickMoveArea(int[] labels, Map<Integer, RegionNew> regionLists, Map<Integer, Integer> regionSpatialAttrs, List<Integer> spatially_extensive_attr, SpatialGrid r, int[][] distanceMatrix, int threshold){
        List<Integer> potentialAreas = new ArrayList<Integer>();
    ...
    public static long calculateWithinRegionDistance(Map<Integer, RegionNew> regionList, long[][] distanceMatrix){
