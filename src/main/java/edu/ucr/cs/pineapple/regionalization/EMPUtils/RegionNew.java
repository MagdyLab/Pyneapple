...
public class RegionNew {
    ...
    public static void setRange(Double minLowerBound,
        ...
        RegionNew.minLowerBound = minLowerBound;
        RegionNew.minUpperBound = minUpperBound;
        RegionNew.maxLowerBound = maxLowerBound;
        RegionNew.maxUpperBound = maxUpperBound;
        RegionNew.avgLowerBound = avgLowerBound;
        RegionNew.avgUpperBound = avgUpperBound;
        RegionNew.sumLowerBound = sumLowerBound;
        RegionNew.sumUpperBound = sumUpperBound;
        RegionNew.countLowerBound = countLowerBound;
        RegionNew.countUpperBound = countUpperBound;
    }
    //Set<Integer>
    public RegionNew(int id){
        numOfAreas = 0;
        average = 0;
        this.id = id;
        this.acceptLow = RegionNew.avgLowerBound;
        this.acceptHigh = RegionNew.avgUpperBound;
    ...
    public boolean addArea(Integer id, long minAttrVal, long maxAttrVal, long avgAttrVal, long sumAttrVal, SpatialGrid sg){
        if (areaList.contains(id)){
            ...
            acceptLow = RegionNew.avgLowerBound * (numOfAreas + 1) - average * numOfAreas;
            acceptHigh = RegionNew.avgUpperBound * (numOfAreas +1) - average * numOfAreas;
    ...
    public boolean removable(Integer area, ArrayList<Long> minAttr, ArrayList <Long> maxAttr, ArrayList<Long> avgAttr, ArrayList<Long> sumAttr, SpatialGrid sg){
        ...
        if((this.numOfAreas - 1) >= RegionNew.countLowerBound &&
                ((this.sum - sumAttr.get(area)) >= RegionNew.sumLowerBound) &&
                ((this.average * numOfAreas - avgAttr.get(area)) / (numOfAreas - 1) >= RegionNew.avgLowerBound) &&
                ((this.average * numOfAreas - avgAttr.get(area)) / (numOfAreas - 1) <= RegionNew.avgUpperBound)){
            if(this.min == minAttr.get(area) || this.max == maxAttr.get(area)){
                ...
                if(tmpMin <= RegionNew.minUpperBound && tmpMin >= RegionNew.minLowerBound && tmpMax <= RegionNew.maxUpperBound && tmpMax >= RegionNew.maxLowerBound){
    ...
    public RegionNew mergeWith(RegionNew expandR,  ArrayList<Long> minAttr, ArrayList<Long> maxAttr, ArrayList<Long> avgAttr, ArrayList<Long> sumAttr,SpatialGrid sg) {
        if(expandR == null){
        ...
        RegionNew tmpR = new RegionNew(-1);
